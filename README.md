...
- ノード `A` は `echo "データ取得開始" && sleep 1` を実行します。
- ノード `B` は `echo "前処理中..."` を実行します。
- ノード `C` は `exec` コメントがないため、デフォルトでノード名 "モデル訓練" を出力します。

## 今後の展望

- [x] DAGファイルからパイプラインを動的に作成して実行する機能の実装
- [ ] エラーハンドリングの強化
- [ ] 実行ログの保存機能
